...
| `RAILWAY_PUBLIC_DOMAIN` | set by Railway            | Used verbatim as the host in every generated link (`externalHostFor()`); falls back to the request's `Host` header if unset (e.g. local dev). Not something you set yourself. |
