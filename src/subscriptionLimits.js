...
// key/value table" pattern as src/modes.js. Defaults to unlimited in
// both dimensions, per user request -- a fork/deployment where the
// admin never opens this section behaves exactly as before (no
// expiry, no usage cap).
//
...
/**
 * Current admin-set limits. `days`/`usageGB` are null when unlimited
 * (the default -- see module header). `setAt` is the ISO timestamp
 * the limits were last saved, used as the countdown's start point;
 * null if never saved.
 */
function getLimits() {
  ...
  return {
    days: isUnsetOrEmpty(daysRaw) ? null : Number(daysRaw),
    usageGB: isUnsetOrEmpty(usageRaw) ? null : Number(usageRaw),
    setAt,
  };
}
...
/**
 * Days/usage remaining given the current limits and a total-used byte
 * count (see inbounds.js's getTotalTrafficBytes()). Either value is
 * null when that dimension is unlimited. Never negative -- floors at
 * 0 once a limit is exceeded (this module doesn't act on that, just
 * reports it -- see module header's scope note).
 */
function getUsageSummary(totalUsedBytes) {
  ...
  return { daysLeft, usageLeftBytes, unlimitedDays, unlimitedUsage };
}
